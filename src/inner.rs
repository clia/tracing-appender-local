...
impl InnerAppender {
    ...
    fn write_timestamped(&mut self, buf: &[u8], date: DateTime<Local>) -> io::Result<usize> {
    ...
    fn refresh_writer(&mut self, now: DateTime<Local>) {
    ...
    fn should_rollover(&self, date: DateTime<Local>) -> bool {
        date >= self.next_date
    }
}
