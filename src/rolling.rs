...
use chrono::{DateTime, Datelike, TimeZone, Timelike, Local};
use std::fmt::Debug;
use std::io;
use std::path::Path;
...
impl RollingFileAppender {
    ...
    /// and hour (Local format) to the file name.
    ///
...
/// adds the current date, hour, and minute to the log file in Local.
///
/// # Examples
///
/// ``` rust
...
/// The directory of the log file is specified with the `directory` argument.
/// `file_name_prefix` specifies the _prefix_ of the log file. `RollingFileAppender`
/// adds the current date and hour to the log file in Local.
///
/// # Examples
///
/// ``` rust
...
/// defined by [`Rotation`]. The `directory` and
/// `file_name_prefix` arguments determine the location and file name's _prefix_
/// of the log file. `RollingFileAppender` automatically appends the current date in Local.
///
/// # Examples
///
/// ``` rust
...
impl Rotation {
    ...
    pub(crate) fn next_date(&self, current_date: &DateTime<Local>) -> DateTime<Local> {
        let unrounded_next_date = match *self {
            ...
            Rotation::NEVER => Local.ymd(9999, 1, 1).and_hms(1, 0, 0),
        };
        self.round_date(&unrounded_next_date)
    }

    pub(crate) fn round_date(&self, date: &DateTime<Local>) -> DateTime<Local> {
        match *self {
            Rotation::MINUTELY => Local.ymd(date.year(), date.month(), date.day()).and_hms(
                date.hour(),
                date.minute(),
                0,
            ),
    ...
    pub(crate) fn join_date(&self, filename: &str, date: &DateTime<Local>) -> String {
        match *self {
...
mod test {
    ...
    fn test_next_date_minutely() {
        ...
        let mock_now = Local.ymd(2020, 2, 1).and_hms(0, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_minute(1).unwrap(), next);

        let mock_now = Local.ymd(2020, 2, 1).and_hms(0, 20, 30);
        let next = r.next_date(&mock_now);
        assert_eq!(
            mock_now
                .with_hour(0)
        ...
        let mock_now = Local.ymd(2020, 2, 1).and_hms(0, 59, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_hour(1).unwrap().with_minute(0).unwrap(), next);

        let mock_now = Local.ymd(2020, 2, 1).and_hms(23, 59, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(
            mock_now
                .with_day(2)
        ...
        let mock_now = Local.ymd(2020, 12, 31).and_hms(23, 59, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(Local.ymd(2021, 1, 1).and_hms(0, 0, 0), next);
    }
    ...
    fn test_next_date_hourly() {
        ...
        let mock_now = Local.ymd(2020, 2, 1).and_hms(0, 20, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_hour(1).unwrap().with_minute(0).unwrap(), next);

        let mock_now = Local.ymd(2020, 2, 1).and_hms(1, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_hour(2).unwrap(), next);

        let mock_now = Local.ymd(2020, 2, 1).and_hms(23, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_day(2).unwrap().with_hour(0).unwrap(), next);

        let mock_now = Local.ymd(2020, 12, 31).and_hms(23, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(Local.ymd(2021, 1, 1).and_hms(0, 0, 0), next);
    }
    ...
    fn test_next_date_daily() {
        ...
        let mock_now = Local.ymd(2020, 8, 1).and_hms(0, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_day(2).unwrap().with_hour(0).unwrap(), next);

        let mock_now = Local.ymd(2020, 8, 1).and_hms(0, 20, 5);
        let next = r.next_date(&mock_now);
        assert_eq!(Local.ymd(2020, 8, 2).and_hms(0, 0, 0), next);

        let mock_now = Local.ymd(2020, 8, 31).and_hms(11, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(Local.ymd(2020, 9, 1).and_hms(0, 0, 0), next);
    ...
    fn test_round_date_minutely() {
        let r = Rotation::MINUTELY;
        let mock_now = Local.ymd(2020, 2, 1).and_hms(10, 3, 1);
        assert_eq!(
            Local.ymd(2020, 2, 1).and_hms(10, 3, 0),
            r.round_date(&mock_now)
        );

        let mock_now = Local.ymd(2020, 2, 1).and_hms(10, 3, 0);
    ...
    fn test_round_date_hourly() {
        ...
        let mock_now = Local.ymd(2020, 2, 1).and_hms(10, 3, 1);
        assert_eq!(
            Local.ymd(2020, 2, 1).and_hms(10, 0, 0),
            r.round_date(&mock_now)
        );

        let mock_now = Local.ymd(2020, 2, 1).and_hms(10, 0, 0);
        assert_eq!(mock_now, r.round_date(&mock_now));
    }
    ...
    fn test_round_date_daily() {
        ...
        let mock_now = Local.ymd(2020, 2, 1).and_hms(10, 3, 1);
        assert_eq!(
            Local.ymd(2020, 2, 1).and_hms(0, 0, 0),
            r.round_date(&mock_now)
        );
    ...
    fn test_next_date_never() {
        ...
        let mock_now = Local.ymd(2020, 2, 1).and_hms(0, 0, 0);
        let next = r.next_date(&mock_now);
        assert_eq!(next, Local.ymd(9999, 1, 1).and_hms(1, 0, 0));
    }
